...
re-coded or modified by ansanshif
to bring new ai
*/
...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to professor voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('professor') && conf.TALKING_professor !== 'true') {        
        ...
        var finm = message.message.replace('professor', '').replace(' ', '')   
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_professor == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel professor özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'professor yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'professor yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'professor, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'professor, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional professor features. Turn your account into a ai chatbot!'
    already_on = 'professor artificial intelligence is already fully functional.'
    already_off = 'professor artificial intelligence is currently running semi-functional.'
    succ_on = 'professor Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'professor Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ professor സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'professor കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'professor AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'professor പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി professor സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'professor ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.professor on / off' }, (async (message, match) => {
    var professor_status = `${conf.TALKING_professor}`
    if (match[1] == 'on') {
        if (professor_status == 'true') {
            return await message.client.sendMessage(message.jid, '*' + already_on + '*', MessageType.text)
        }
        else {
            await heroku.patch(baseURI + '/config-vars', { 
                body: { 
                    ['TALKING_professor']: 'true'
                } 
            });
